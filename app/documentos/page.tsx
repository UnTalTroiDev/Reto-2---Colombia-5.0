...
export default async function DocumentosPage() {
  ...
  const max = Number(statsTam?.mx ?? 0);
  const min = Number(statsTam?.mn ?? 0);
  const avg = Number(statsTam?.avg ?? 0);
      ...
      <section>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-px bg-[var(--color-border)]">
          ...
          <StatCard
            ...
            hint={`${((nullDescr / rowsTotal) * 100).toFixed(4)}%`}
            tone={nullDescr / rowsTotal > 0.05 ? "warn" : "success"}
          />
        </div>
      </section>
                ...
                <li key={d.key} className="flex items-center justify-between">
                  <span className="flex items-center gap-2">
                    <span
                      className="size-2"
                      style={{
                        background: ["#e0623a", "#d4a64a", "#6ba368", "#5b8db5", "#a16ba1", "#c75450"][i],
                      }}
                      aria-hidden
                    />
